...
export class AlchemyMultichainClient {
   ...
   */
  constructor(overrides: Partial<Record<Network, AlchemyMultichainSettings>>) {
  ...
  private loadInstance(network: Network): Alchemy {
    if (!this.instances.has(network)) {
      ...
      const alchemySettings = this.overrides &&
        this.overrides[network] && { ...this.overrides[network], network };
